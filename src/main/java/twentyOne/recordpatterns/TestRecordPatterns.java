package twentyOne.recordpatterns;
...
//        nonNesting(p2);  // Mary Bloggs; 23
    ...
    public static void nesting(Doubles dbles) {
        ...
        if (dbles instanceof Doubles(Player pl1, Player pl2)) {   // LVTI works also (Local Variable Type Inference)
            System.out.println(pl1.person() + "; " + pl1.ability());
            System.out.println(pl2.person() + "; " + pl2.ability());
        }
        // more complex nesting
        if (dbles instanceof Doubles(Player(Person p1, Ability ability), var pl2)) { // LVTI works also (Local Variable Type Inference)
            System.out.println(p1.name() + "; " + p1.age() + "; " + ability.name() + "; " + ability);
